...
GraflowはTaskWorkerを用いた分散並列処理を標準装備し、CLI一発でワーカー群を立ち上げられます。
...
- **組み込みCLIワーカー**: `python -m graflow.worker.main --worker-id ...`
...
| モード | 動作 | 典型ユースケース |
|--------|------|----------------|
| `strict` | 1つ失敗で全体停止 | 重要業務ワークフロー |
| `lenient` | 成功分は先へ進む | 大規模バッチ処理 |
| `quorum` | M/Nで成功扱い | アンサンブル推論 |
| `bounded_retries` | 再試行回数を制限 | 不安定な外部API |
| `custom` | ユーザー定義 | 特殊要件 |

ライブ統計に基づき、失敗時の自動リトライ／スキップを制御できます。

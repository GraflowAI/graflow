...
def test_engine_execution_with_dynamic_tasks():
    ...
    # Create context AFTER graph is built
    ...
    assert "start_task" in context.completed_tasks
    assert "dynamic_urgent" in context.completed_tasks
    assert "successor_task" in context.completed_tasks
    assert "final_task" in context.completed_tasks

    # Verify execution order - dynamic task should execute before successors
    executed_order = list(context.completed_tasks)
...
def test_multiple_dynamic_tasks():
    ...
    # Verify all tasks executed
    assert "coordinator" in context.completed_tasks
    assert "worker_0" in context.completed_tasks
    assert "worker_1" in context.completed_tasks
    assert "worker_2" in context.completed_tasks
    assert "cleanup" in context.completed_tasks
...
def test_dynamic_tasks_with_processing():
    ...
    # Verify all tasks executed
    assert "processor" in context.completed_tasks
    assert "analysis_0" in context.completed_tasks
    assert "analysis_1" in context.completed_tasks
    assert "analysis_2" in context.completed_tasks
    assert "final_report" in context.completed_tasks
...
def test_jump_to_existing_task_nodes():
    ...
    def start_node_func(task_ctx):
        ...
        jump_to_task = task_ctx.graph.get_node("existing_task")
    ...
    def existing_task_func(task_ctx):
        ...
        other_task = task_ctx.graph.get_node("other_existing")
    ...
    assert "start_node" in context.completed_tasks
    assert "existing_task" in context.completed_tasks
    assert "other_existing" in context.completed_tasks

    # Verify successors were skipped due to goto behavior
    assert "successor_node" not in context.completed_tasks
    assert "final_node" not in context.completed_tasks
...
def test_complex_task_jumping():
    ...
    def controller_func(task_ctx):
        ...
        for task_name in tasks_to_execute:
            existing_task = task_ctx.graph.get_node(task_name)
    ...
    def analyzer_func(task_ctx):
        ...
        processor_task = task_ctx.graph.get_node("data_processor")
    ...
    actually_executed = set(context.completed_tasks)

...
from graflow.utils.graph import build_graph, draw_task_graph
...
def test_parallel_group_graph():
    ...
    assert set(graph.nodes()) == {group.name, "A", "B"}
    expected_edges = {(group.name, "A"), (group.name, "B")}
...
def test_merge_parallel_groups():
    ...
    expected_nodes = {"A", "B", "C", merged_group.name}
...
def test_merge_parallel_groups2():
    ...
    expected_nodes = {"A", "B", "C", "D", "E", pipeline.name}
    assert set(graph.nodes()) == expected_nodes
    expected_edges = {
        (pipeline.name, "A"),
        (pipeline.name, "B"),
...
def test_complex_graph2():
    ...
    expected_nodes = {"A", "B", "C", "D", "E", "F", group1.name, group2.name}
    assert set(graph.nodes()) == expected_nodes
    expected_edges = {
        ("A", group1.name),
        (group1.name, "B"), (group1.name, "C"),
        ("B", group2.name), ("C", group2.name),
        (group2.name, "D"), (group2.name, "E"), (group2.name, "F"),
        }
    assert set(graph.edges()) == expected_edges
